...
{
    static float last_distance = 1000.0f;   // 上次距离
    static int notok_count = 0;         // 连续失败检测次数
    static bool tof_ok = true;       // 是否正常

    tof.upData();
    tof.thread_syn();
    uint16_t raw = tof.syn_vl53l0x_distance_mm;

    if (raw == 0 || raw == 8192 || raw > 2000){
        tof_ok = false;
    }

    if (tof_ok){
        notok_count = 0;
        float get_distance = (float)(raw);

        last_distance = 0.7 * last_distance + 0.3 * get_distance;  // 一阶低通滤波
        ...
        // 长时间数据无效，重置为远距离
            ...
            tof_distance = 1000.0f;  // 超过5次异常，重置距离
            last_distance = 1000.0f;
            notok_count = 0;
        }
    }

    // 限幅
    if (tof_distance >= 1000.0f) tof_distance = 1000.0f; 
    if (tof_distance <= 50.0f) tof_distance = 50.0f;    

    // 串口输出调试　待删除
    std::cout << "TOF测距: " << tof_distance 
              << "TOF原始数据 = " << raw
              << "TOF有效标志位 = " << tof_ok
              << std::endl;
}
...
void all_angle_get(void)
{
    ...
    pitch_angle = madg.pitch_get();   //获取俯仰角
    roll_angle = madg.roll_get();     //获取翻滚角
}
...
void motor_distance_get(void)
{
    ...
    motor_distance += (L_speed + R_speed) / 392.0f;   //单位cm  
...
void state_reset(void)
{
    ...
    //屏幕提醒
    remind.is_screen_remind = false;
    remind.is_screen_clear = false;
