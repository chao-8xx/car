# -*- coding: utf-8 -*-
"""
06_augment_right_circle_to_20000_v1.py
...
5. 将右圆环 train 精确规划到约 20000 张，并让六小类接近平衡；
...
按当前右圆环 train 数量：
supplies/medkit       260
supplies/telescope    790
transport/ambulance   746
transport/armored_car 578
weapon/explosive      380
weapon/gun            504
...
INPUT_SPLIT_ROOT = r"E:\code\Dataset_Split_mytrain_new\enhance\fenzu\圆环右"
OUTPUT_ROOT = r"E:\code\Dataset_Split_mytrain_new\enhance\圆环右"
...
def apply_downscale_restore(image, rng: random.Random):
    ...
    if level == "light":
        scale = rng.uniform(0.72, 0.90)
    elif level == "medium":
        scale = rng.uniform(0.52, 0.72)
    else:
        scale = rng.uniform(0.40, 0.52)
...
def apply_side_perspective(image, rng: random.Random):
    ...
    if level == "light":
        ...
        ratio = rng.uniform(0.07, 0.10)
    else:
        ratio = rng.uniform(0.10, 0.14)
...
def apply_anisotropic_compress(image, rng: random.Random):
    ...
    if rng.random() < 0.70:
        # 右圆环侧视仍以水平方向压缩为主。
        scale_x = rng.uniform(0.66, 0.92)
        scale_y = rng.uniform(0.94, 1.04)
    else:
        scale_x = rng.uniform(0.92, 1.04)
        scale_y = rng.uniform(0.76, 0.95)
