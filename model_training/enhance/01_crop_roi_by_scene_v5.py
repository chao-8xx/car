...
SIDE_MIN_RED_PIXEL_RATIO = 0.18
SIDE_INNER_WHITE_MIN = 0.12
SIDE_VERTICAL_WHITE_MIN = 0.08
...
def build_red_mask(img_bgr):
    ...
    # 侧视红条较小，形态学核不能像普通场景那样过大，否则会把蓝白边缘粘连起来。
...
def find_red_marker(img_bgr):
    ...
    for cnt in contours:
        ...
        if SIDE_CONTEXT:
            ...
            if inner_white < SIDE_INNER_WHITE_MIN:
                continue
            if vertical_white < SIDE_VERTICAL_WHITE_MIN:
                continue
        ...
        if SIDE_CONTEXT:
            ...
            center_score = (
                center_score * 0.8
                + min(side_red_ratio / 0.60, 1.0) * 1.8
                + min(inner_white / 0.50, 1.0) * 1.2
                + min(vertical_white / 0.40, 1.0) * 0.8
            )
        else:
            bounds = estimate_lane_bounds(img_bgr, cy)
            if bounds is None:
