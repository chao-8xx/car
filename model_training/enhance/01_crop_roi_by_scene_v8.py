...
SOURCE_DIR = r"E:\code\dataset_all\数据集_新圆环_7.13\右"
...
SIDE_CROP_SIDE_SCALE = 1.35
SIDE_CROP_MIN_SIDE = 20
...
ROI_MIN_WHITE_RATIO = 0.18
ROI_MIN_RED_RATIO = 0.006
ROI_MAX_BLUE_RATIO = 0.55
ROI_MAX_SKIN_RATIO = 0.24

# 侧视场景始终沿X方向裁剪；同时评估红框左右两侧，优先选真正目标板一侧。
SIDE_TRY_BOTH_DIRECTIONS = True
SIDE_OPPOSITE_SWITCH_MARGIN = 0.35
SIDE_CANDIDATE_MIN_SCORE = 0.45
...
def find_red_marker(img_bgr):
    ...
    for cnt in contours:
        ...
        if SIDE_CONTEXT:
            vertical_position_score = 1.0 - min(
                abs(y_ratio - 0.42) / 0.42,
                1.0,
            )
        else:
            vertical_position_score = y_ratio
    ...
    # 侧视场景不能只凭红色轮廓决定。
    # 对排名靠前的候选实际做X方向裁剪，选择最像目标板、最不像手臂/蓝边的那个。
    reranked = []

    for base_score, box in candidates[:8]:
        crop_img, _, _ = normalize_crop_by_scene(img_bgr, box)
        if crop_img is None or crop_img.size == 0:
            continue

        quality_ok, metrics = evaluate_roi_quality(crop_img)
        q_score = roi_quality_score(metrics)

        # y越靠画面底部越容易是车身/蓝边杂物，侧视候选不再奖励“越低越好”。
        _, yy, _, hh = box
        candidate_y = (yy + hh * 0.5) / float(max(1, h_img))
        y_score = 1.0 - min(abs(candidate_y - 0.42) / 0.42, 1.0)

        final_score = base_score * 0.35 + q_score * 2.2 + y_score * 0.8

        if quality_ok or final_score >= SIDE_CANDIDATE_MIN_SCORE:
            reranked.append((final_score, box))

    if not reranked:
        return None, mask

    reranked.sort(key=lambda item: item[0], reverse=True)
    return reranked[0][1], mask
...
def crop_target_side_marker(img_bgr, red_box, crop_dir):
    """
    沿X方向裁剪：
    crop_dir < 0：取红框左侧；
    crop_dir > 0：取红框右侧。
...
def roi_quality_score(metrics):
    """
    分数越高越像“目标板ROI”：
    白色和少量红色加分，蓝色和皮肤区域扣分。
    """
    white = metrics["white_ratio"]
    red = metrics["red_ratio"]
    blue = metrics["blue_ratio"]
    skin = metrics["skin_ratio"]

    return (
        min(white / 0.45, 1.0) * 1.6
        + min(red / 0.10, 1.0) * 1.4
        - min(blue / 0.60, 1.0) * 1.2
        - min(skin / 0.35, 1.0) * 2.0
    )


def rotate_side_crop_if_needed(crop_img, red_box, crop_dir):
    """
    仍保留最小判断：
    红框竖向(h>w)才旋转90度；横向不旋转。
    旋转方向由实际选择的裁剪侧决定。
    """
    _, _, w, h = red_box
    if crop_img is None:
    ...
    if crop_dir < 0:
        return cv2.rotate(crop_img, cv2.ROTATE_90_CLOCKWISE), "cw90"
    return cv2.rotate(crop_img, cv2.ROTATE_90_COUNTERCLOCKWISE), "ccw90"


def normalize_crop_by_scene(img_bgr, red_box):
    """
    V8：
    - 普通直道/普通弯道：向红框上方裁，不动原逻辑；
    - 左右圆环/急弯：始终沿X方向裁剪；
    - 先尝试文件夹对应方向，再评估红框另一侧；
    - 如果另一侧明显更像目标板，则自动切换，避免把拍摄者手臂裁进去；
    - 红框竖向时才旋转90度。
    """
    if not SIDE_CONTEXT:
    ...
    candidate_dirs = [SIDE_DIR]
    if SIDE_TRY_BOTH_DIRECTIONS:
        candidate_dirs.append(-SIDE_DIR)

    choices = []

    for crop_dir in candidate_dirs:
        raw_crop, crop_box = crop_target_side_marker(
            img_bgr,
            red_box,
            crop_dir,
        )
        ...
        crop_img, rotate_text = rotate_side_crop_if_needed(
            raw_crop,
            red_box,
            crop_dir,
        )
        ...
        quality_ok, metrics = evaluate_roi_quality(crop_img)
        score = roi_quality_score(metrics)

        # 文件夹给出的期望方向只给一个小奖励，不允许它压过明显更好的目标板ROI。
        if crop_dir == SIDE_DIR:
            score += 0.20
    ...
    # 如果首选方向和文件夹方向不同，但只好一点点，仍保持文件夹方向，减少方向抖动。
    expected = next(
        (item for item in choices if item["crop_dir"] == SIDE_DIR),
        None,
    )
    if (
        expected is not None
        and best["crop_dir"] != SIDE_DIR
        and best["score"] < expected["score"] + SIDE_OPPOSITE_SWITCH_MARGIN
    ):
        best = expected

    direction_text = "L" if best["crop_dir"] < 0 else "R"
    rotate_text = f"{direction_text}_{best['rotate_text']}"

    return best["crop_img"], best["crop_box"], rotate_text
